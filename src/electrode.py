...
class Electrode:
    ...
    # Gives you the event rate per minute considering events iff it is of any
    # type of the ones listed in event_types
    def get_events_rate(self, event_types=None):
        ...
        # Note: rate[1] is duration, may be None if no hfo was registered for
        # that block
        block_rates_arr = [
    ...
    def has_epileptic_activity(self, event_types=None):
        if not self.soz:
            return False
        else:  # it is soz channel, has events?
            if event_types is None:
                event_types = ['RonO', 'RonS', 'Spikes', 'Fast RonO',
                               'Fast RonS', 'Sharp Spikes']
            for event_type in event_types:
    ...
    def has_nsoz_activity(self, event_types=None):
        if self.soz:
            return False
        else:  # it is soz channel, has events?
            if event_types is None:
            ...
            for event_type in event_types:
                for block, count in self.evt_count[event_type].items():
                    if count > 0:
                        return True
            return False  # empty nsoz channel
    ...
    def remove_rand_evt(self, hfo_type, art_radius=20):
        ...
        for i, evt in enumerate(self.events[hfo_type], start=0):
            if (artifact_freq - art_radius) <= evt.info['freq_av'] <= (artifact_freq + art_radius):
    ...
    def print(self):
        ...
        # print('\t\t\tEvents: {0}'.format([{type_name: len(l) for type_name,
        # l in self.events.items()}]))
