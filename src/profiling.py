...
# memory profiler --> from profiling import profile_memory . Use as decorator
# @profile_memory
# time profiler --> basic . profiling.function_timer(f)
# time_profiler --> basic with checkpoints (fav)Use as :  with
# profiling.timer_for('My function') as timer:
# time profiler Clib --> profiling.profile_time(f)
...
def function_timer(f):
    def f_timer(*args, **kwargs):
        ...
        print('{fname} took {elapsed} time'.format(fname=f.__name__,
                                                   elapsed=end - start))
        return result
